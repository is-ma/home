...
THIS_FILE="$HOME/.config/i3blocks/scripts/memory"
POINTS_TO="$TARGETS/i3blocks/scripts/memory"
ln -sf "$POINTS_TO" "$THIS_FILE"

THIS_FILE="$HOME/.config/i3blocks/scripts/cpu_usage"
POINTS_TO="$TARGETS/i3blocks/scripts/cpu_usage"
ln -sf "$POINTS_TO" "$THIS_FILE"

THIS_FILE="$HOME/.config/i3blocks/scripts/weather"
...
for plugin_dir in "$PWD/targets/albert/python/plugins"/*; do
    if [ -d "$plugin_dir" ]; then
        ...
        rm -rf "$HOME/.local/share/albert/python/plugins/$plugin_name"
